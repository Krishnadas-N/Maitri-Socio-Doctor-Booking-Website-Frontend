...
export class DoctorProfilePageComponent implements OnInit, OnDestroy {
  ...
  getSimilarProfiles() {
    const specializationId = this.getSpecializationId(
    ...
    this.commonService.getSimilarProfiles(specializationId).subscribe({
      next: (res) => {
        ...
        this.similarProfileDoctors = res.data.filter(
          (data: any) => data._id !== this.doctorDetails._id
        );
      },
      error: (err) => {
        console.log(err);
  ...
  getDoctorPosts() {
    if (this.doctorDetails && this.doctorDetails?._id) {
      this.postsSubscription = this.feedService
        .getPostsOfDoctorById(this.doctorDetails._id)
        .subscribe({
          next: (res) => {
  ...
  getNextDoctor() {
    if (
      this.currentCountOfSimilarProfileDoctors >=
      this.similarProfileDoctors.length - 1
    ) {
      return;
    } else {
      this.currentCountOfSimilarProfileDoctors++;
