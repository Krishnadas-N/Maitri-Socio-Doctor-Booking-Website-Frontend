...
export class UserSideAppoinmentListingComponent implements OnInit {
  ...
  fetchUserAppoinmentDetails() {
    this.userService
      .getUserAppoinments(this.currentPage, this.pageSize)
      .subscribe({
        next: (res: any) => {
  ...
  changeStatus(event: any): void {
    ...
    this.userService
      .requestTochangeAppoinmentStatus(event.id, event.reason, event.status)
      .subscribe({
        next: (res: any) => {
          console.log(res.data);
          const index = this.userAppoinmentDetails.findIndex(
            (appointment: any) =>
              appointment._id.toString() === event.id.toString()
          );
