...
export class TableComponent implements OnInit {
  ...
  getStatusClass(column: string, value: any): string {
    switch (column) {
      case 'isBlocked':
        return value ? 'bg-green-500 text-white rounded-full px-2 py-1' : 'bg-red-500 text-white rounded-full px-2 py-1';
      case 'isVerified':
        return value ? 'bg-green-500 text-white rounded-full px-2 py-1' : 'bg-red-500 text-white rounded-full px-2 py-1';
      default:
        return '';
  ...
  blockUser(row: any) {
    ...
    console.log(`User ${row.username} ${row.isBlocked ? 'unblocked' : 'blocked'}`);
  }
  confirm(): void {
