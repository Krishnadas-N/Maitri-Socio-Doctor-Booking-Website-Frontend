...
export class PostCommentComponent implements OnInit {
  ...
  replyToTargetComment(comment: any, replyText: string): void {
    ...
    this.feedService
      .replyToComment(this.postData.post._id as string, comment._id, replyText)
      .subscribe({
        next: (res: any) => {
          ...
          console.log(comment);
          this.postData.post.comments
            .find((c: any) => c._id.toString() === comment._id.toString())!
  ...
  postNewComment(commentText: string): void {
    if (commentText.trim() !== '' && this.postData.post._id) {
      ...
      this.isLoading = true;
      this.feedService
        .commentOnPost(this.postData.post._id as string, commentText.trim())
        .subscribe({
          next: (res: any) => {
            this.isLoading = false;
            console.log(res);
  ...
  toggleRepliesVisibility(comment: any): void {
    ...
    if (!(comment._id in this.commentRepliesVisibility)) {
      ...
      this.commentRepliesVisibility[comment._id] =
        !this.commentRepliesVisibility[comment._id];
    }
  }
