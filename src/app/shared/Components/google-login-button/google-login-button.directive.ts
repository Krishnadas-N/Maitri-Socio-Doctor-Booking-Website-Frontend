...
export class GoogleLoginButtonComponent implements OnInit {
  ...
  @Input() text: 'signin_with' | 'signup_with' | 'continue_with' =
    'signin_with';
  ...
  ngOnInit(): void {
    ...
    if (this.type === 'standard' && ['square', 'circle'].includes(this.shape)) {
      throw new Error(
        'Invalid shape for standard type button. Valid shapes: rectangular, pill.'
      );
    }

    if (this.type === 'icon' && ['rectangular', 'pill'].includes(this.shape)) {
      throw new Error(
        'Invalid shape for icon type button. Valid shapes: square, circle.'
      );
    }

    if (this.width && (this.width < 200 || this.width > 400)) {
      throw new Error(
        'Invalid button width. Valid width: min 200 and max 400.'
      );
    }
  ...
  renderButton = () => {
    // @ts-ignore
    window.google.accounts.id.renderButton(this.el.nativeElement, {
      locale: this.locale,
